...
public class AdminController {
    ...
    // --- 4. GLOBAL CODE REPOSITORY (View All Files) ---
    @GetMapping("/admin/repository")
    public String showCodeRepository(HttpSession session, Model model) {
        User user = (User) session.getAttribute("loggedInUser");
        if (user == null || !"ADMIN".equals(user.getRole())) {
            return "redirect:/login";
        }
        ...
        model.addAttribute("files", codeFileRepository.findAll());
        return "admin_repository"; // We will create this next
    }
}